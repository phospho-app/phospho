    ...
    for message in messages:
        ...
        for event_name, result in results.items():
            ...
            prediction = await create_prediction(
                message.metadata["task"].org_id,
                message.metadata["task"].project_id,
                phospho_job_id,
                result.value,
                "event_detection",
                task_id=message.metadata["task"].id,
...
async def task_event_detection_pipeline(
    ...
    events_per_task = await run_event_detection_pipeline(
        workload=workload, tasks=[task]
    )

    return events_per_task[task.id]
